...
-- God Aspects: alternate kits that change playstyle / itemization
CREATE TABLE IF NOT EXISTS god_aspects (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    god_id          INTEGER NOT NULL REFERENCES gods(id) ON DELETE CASCADE,
    name            TEXT NOT NULL,     -- e.g. Aspect of Thermotherapy
    description     TEXT,              -- playstyle summary from wiki
    image           TEXT,
    raw_json        TEXT,
    scraped_at      TEXT NOT NULL DEFAULT (datetime('now')),
    UNIQUE (god_id, name)
